/**
 * 加密工具 - MD5实现
 * 基于example中的md5.js改写为TypeScript
 */
 ...
 */
function binl2b64(bin: number[]): string {
  const tab = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  let str = "";
  for (let i = 0; i < bin.length * 4; i += 3) {
    const triplet = (((bin[i >> 2] >> 8 * (i % 4)) & 0xFF) << 16)
                  | (((bin[i + 1 >> 2] >> 8 * ((i + 1) % 4)) & 0xFF) << 8)
                  | ((bin[i + 2 >> 2] >> 8 * ((i + 2) % 4)) & 0xFF);
    for (let j = 0; j < 4; j++) {
      if (i * 8 + j * 6 > bin.length * 32) str += b64pad;
